 ...
 * 在实现上，和 Permutations不同的是要先排序，然后在添加一个元素时，判断这个元素是否等于前一个元素，如果等于，
 ...
 * @time. 2018年4月13日
 */
public class Leetcode47_Backtracking {

	public static void main(String[] args) {
		int[] nums = { 1, 1, 2 };
		List<List<Integer>> list = new ArrayList<List<Integer>>();
		list = new Leetcode47_Backtracking().permuteUnique(nums);
		list.forEach(x -> {
			String s = x.stream().map(i -> i.toString()).collect(Collectors.joining(",", "[", "]"));
	...
	private void backtracking(List<Integer> permuteList, boolean[] visited, int[] nums, List<List<Integer>> ret) {
		...
		for (int i = 0; i < visited.length; i++) {
			 ...
			 * 在i=1中，！[0]意味着我们正在进行下一个迭代，我们不需要再考虑[1]+置换({1，1，2，2})。
			 * 在i=2，！[1]意味着我们已经考虑过类似的排列。
