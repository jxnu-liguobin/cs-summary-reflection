 ...
 * @version v1.0
 */
class TimeoutAction @Inject()(parser: BodyParsers.Default, actorSystem: ActorSystem, configuration: Configuration)
  ...
  override def invokeBlock[A](request: Request[A], block: Request[A] ⇒ Future[Result]): Future[Result] = {
    val timeout = configuration.getOptional[Duration]("api.timeout")
      .map(_.toSeconds.seconds).getOrElse(30.seconds)
    val futureResponse = block(request)
    val futureTimeout = after(timeout, actorSystem.scheduler)(Future.successful {
      implicit val locale: Locale = request.acceptLanguages.headOption.getOrElse(Lang.defaultLang).toLocale
      val code = "request_timeout"
      RequestTimeout(Json.obj(
        "message" -> code,
        "errors" -> Seq(Json.obj(
          "code" -> code, "message" -> s"${code}_msg"))))
    })
