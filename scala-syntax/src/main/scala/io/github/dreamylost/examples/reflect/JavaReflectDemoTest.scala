...
object JavaReflectDemoTest extends App {
  ...
  def method(): Unit = {
    ...
    val ret = clazz.getDeclaredMethod("testMethod", classOf[String]).invoke(clazz.newInstance(), "test name for args").asInstanceOf[String]
    println(ret)
  }
  ...
  def staticMethod(): Unit = {
    ...
    val ret2 = clazz2.getDeclaredMethod("testStaticMethod", classOf[String]).invoke(null, "test name for args").asInstanceOf[String]
    println(ret2)
  }
}
