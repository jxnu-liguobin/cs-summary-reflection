...
mod test {
    ...
    fn delete_duplicates() {
        let e1 = Some(Box::new(ListNode { val: 3, next: None }));
        let e2 = Some(Box::new(ListNode { val: 3, next: e1 }));
        let e3 = Some(Box::new(ListNode { val: 2, next: e2 }));
        let e4 = Some(Box::new(ListNode { val: 1, next: e3 }));
        let e5 = Some(Box::new(ListNode { val: 1, next: e4 }));
        let mut ret = Solution::delete_duplicates(e5);
        let mut rs = Vec::new();
        while ret.is_some() {
