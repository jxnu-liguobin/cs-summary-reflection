 ...
 *
 * <p>副作用应该是来源于函数式编程。指的是函数调用时，除了返回值，函数还会影响函数外部其他东西（外部变量，I/O，参数内容）。
 * 如果函数没有任何副作用，我们称这个函数为<em>纯函数</em>，否则叫<em>非纯函数</em> 纯函数的优点：1.不需要记住状态变量，心智负担小。2.可以缓存执行结果3.无状态，所以线程安全
 *
 * @author 梦境迷离
 * @version v1.0
 * @since 2019-12-07
