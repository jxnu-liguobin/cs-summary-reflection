...
public class SelectSort extends Constant {
	 ...
	 * 
	 * @author 梦境迷离
	 * @description 直接选择 首先我们选择排序的交换操作介于 0 和 (n - 1） 次之间。选择排序的比较操作为 n (n - 1） /
	 *              2 次之间。选择排序的赋值操作介于 0 和 3 (n - 1） 次之间。
	 *              比较次数O(n^2），比较次数与关键字的初始状态无关，总的比较次数N=(n-1）+(n-2）+...+1=n*(n-1）
	 *              /2。交换次数O(n），最好情况是，已经有序，交换0次；最坏情况交换n-1次，逆序交换n/2次。
	 ...
	 *              9，我们知道第一遍选择第1个元素5会和2交换，那么原序列中两个5的相对前后顺序就被破坏了，
	 *              所以选择排序是一个不稳定的排序算法。
	 * @time 2018年4月8日
	 */
