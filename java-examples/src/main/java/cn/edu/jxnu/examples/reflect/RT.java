...
/**
 * There are 3 bootstrap methods: - one for the constant that are initialized once the first time
 * the bootstrap method is called and after always reuse the same constant. It's almost equivalent
 * to an LDC but here the constant are stored in boxed form e.g a java.lang.Integer containing 0
 * instead of an int containing 0.
 *
 * <p>- one for the unary operation 'not' and 'asBoolean', here the semantics is hard coded, all
 * primitive value are transformed as object by applying this operation: (v == 0)? false: true
 *
 * <p>- one for the binary operation 'add', 'mul' and 'gt', here the semantics can be changed by
 * adding more static methods in {@link BinaryOps}. This bootstrap method is a little more complex
 * because it creates an inlining cache to avoid to recompute the binary method to call if the type
 * of the two arguments doesn't change. Also, if the expression is used a lot and trigger the JIT,
 * it will be able to inline the code of the operation directly at callsite.
 *
 * @author Remi Forax
 */
public class RT {
    ...
    public static CallSite unary(Lookup lookup, String name, MethodType type) {
        ...
        if (name.equals("asBoolean")) {
            ...
            target =
                    MethodHandles.explicitCastArguments(
                            NOT, MethodType.methodType(Object.class, Object.class));
        }
     ...
     *
     * <p>This bootstrap method doesn't install the target method handle directly, because we want
     * to install an inlining cache and we can't create an inlining cache without knowing the class
     * of the arguments. So this method first installs a method handle that will call {@link
     * BinaryOpCallSite#fallback(Object, Object)} and the fallback method will be called with the
     * arguments and thus can install the inlining cache. Also, the fallback has to be bound to a
     * specific callsite to be able to change its target after the first call, this part is done in
     * the constructor of {@link BinaryOpCallSite}.
     */
    ...
    /**
     * A specific callsite that will install an 'inlining cache'. Because we don't know until
     * runtime which method handle to call, the lookup depending on the dynamic type of the argument
     * will be done at runtime when the method {@link #fallback(Object, Object)} is called. To avoid
     * to do this dynamic lookup at each call, the fallback install two guards in front of dispatch
     * call that will check if the arguments class change or not. If the arguments class don't
     * change, the previously computed method handle will be called again. Otherwise, a new method
     * handle will be computed and two new guards will be installed.
     */
            ...
            try {
                ...
                MethodType testType =
                        MethodType.methodType(
                                boolean.class, Class.class, Object.class, Object.class);
    ...
    /**
     * Garbage class that contains the raw operations used for binary operations. All methods must
     * be static returns an Object and takes the same type for the two parameter types.
     *
     * <p>See {@link RT#lookupBinaryOp(String, Class, Class)} for more info.
     */
    ...
    /**
     * Select a most specific method among the ones defined in {@link BinaryOps}. The algorithm
     * first find the most specific subtype between class1 and class2. The order of the types is
     * defined in {@link RT#RANK_MAP}: Boolean < Byte < Short < Character < Integer < Long < Float <
     * Double < String then the algorithm lookup in {@link BinaryOps} to find a method with the name
     * opName taking as argument the primitive corresponding to the most specific subtype. If no
     * such method exist, the algorithm retry but looking for a method with a more specific type
     * (using the same order). The result of the lookup is cached in {@link RT#BINARY_CACHE} to
     * avoid to avoid to do a lookup (a reflective call) on the same method twice.
     */
