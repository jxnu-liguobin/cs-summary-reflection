 ...
 * 在迭代期间面临并发修改时，它提供了快速失败 行为，而不是非确定性行为。 子类是否使用此字段是可选的。
 * 如果子类希望提供快速失败迭代器（和列表迭代器），则它只需在其
 * add(int,e)和remove(int)方法（以及它所重写的、导致列表结构上修改的任何其他方法）中增加此字段。 对add(int,
 * e)或remove(int)的单个调用向此字段添加的数量不得超过 1，否则迭代器（和列表迭代器）将抛出虚假的concurrentmodificationexceptions。 
...
public class ArrayList<E> extends AbstractList<E>
    ...
    /**
     * 空的对象数组，相关：[effective java建议任何时候返回空对象，而不是null]
     */
    ...
    public Object clone() {
        try {
            ...
            v.modCount = 0; // 用于快速失败的modCount设置为初始0，也暗示是新的数组
            // 返回副本v
            return v;
     ...
     * 
     * @param a
     *            如果它的长度大的话，列表元素将存储在这个数组中; 否则，将为此分配一个相同运行时类型的新数组。
    ...
    /**
     * 在ArrayList的移除index位置的元素,会检查去除的位置，返回之前的值
     * 
     * @param index
     *            要删除的元素的索引
     ...
     * 
     * 
     * 注意是比较是equals，也就是说如果是自定义类，需要覆盖equals，hashCode方法
     * @param o
     *            要从该列表中删除的元素（如果存在）
     ...
     * 
     * 2，父子list做的非结构性修改（non-structural
    ...
    static final class ArrayListSpliterator<E> implements Spliterator<E> {
         ...
         * 这允许forEach的内循环在没有任何进一步检查的情况下运行，并且简化了lambda效率。虽然这需要进行多次检查，但请注意，在list.stream（）。
