...
public class Object {
     ...
     * 但是,程序员应该意识到,为不相等的对象生成不同整数结果可以提高哈希表的性能。,若哈希码用在hash中，不同哈希码能降低冲突。
     */
    public native int hashCode();

    /**
     * 这里比较的是对象的内存地址 ，也就是说如果用户不重写这个方法，那么对象的比较都将返回false，因为不同对象的内存地址不可能相同
     */
