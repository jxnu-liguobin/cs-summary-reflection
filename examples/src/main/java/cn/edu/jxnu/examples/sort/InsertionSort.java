...
/**
 * 1、插入排序 插入排序由N-1趟排序组成。对于P=1趟到P=N-1趟，插入排序保证位置0~P-1上的元素是已排序状态。即第N趟排序后，前N个元素是有序的。
 * 插入排序分析 平均复杂度O(n^2),而且这个界是精确的，因为反序输入可以达到该界【大O表示上界】 N个互异的数组，平均逆序数是N(N-1)/4
 * 【2，1，0（逆序=3）】 任何通过交换相邻元素进行排序的任何算法平均需要Ω(N^2)
 * 【欧米伽表示下界，每次只能减少一个逆序，因此需要Ω(N^2)次交换】
