...
object DateUtils {
  ...
  def parseDate(dateStr: String, pattern: String) =
    simpleDateFormat(pattern = pattern).parse(dateStr)
  ...
  )(implicit zoneId: ZoneId = default_zoneId) = {
  ...
  implicit def localDateTimeToString(
      localDateTime: Option[LocalDateTime]
  )(implicit locale: Locale = default_local) = {
    localDateTime match {
  ...
  implicit def toStrNoMS(localDateTime: Option[LocalDateTime]) =
    localDateTimeToStringNoMS(localDateTime)
}
