 ...
 * 
 * 定义一个数组 dp 存储最长递增子序列的长度，dp[n] 表示以 Sn 结尾的序列的最长递增子序列长度。对于一个递增子序列 {Si1,
 * Si2,...,Sim}，如果 im < n 并且 Sim < Sn ，此时 {Si1, Si2,..., Sim, Sn}
 * 为一个递增子序列，递增子序列的长度增加 1。满足上述条件的递增子序列中，长度最长的那个递增子序列就是要找的，在长度最长的递增子序列上加上 Sn 就构成了以
 * Sn 为结尾的最长递增子序列。因此 dp[n] = max{ dp[i]+1 | Si < Sn && i < n} 。
 * 
 * 因为在求 dp[n] 时可能无法找到一个满足条件的递增子序列，此时 {Sn} 就构成了递增子序列，需要对前面的求解方程做修改，令 dp[n] 最小为
 * 1，即：dp[n]=max{1,dp[i]+1 | si<sn && i<n }
 * 
 * 对于一个长度为 N 的序列，最长递增子序列并不一定会以 SN 为结尾，因此 dp[N] 不是序列的最长递增子序列的长度，需要遍历 dp
 * 数组找出最大值才是所要的结果，即 max{ dp[i] | 1 <= i <= N} 即为所求。
 * 
 * @author 梦境迷离.
 * @time 2018年6月9日
 * @version v1.0
...
public class Leetcode_300_Dp {
	 ...
	 * 对于一个元素 x，
	 * 
	 * 如果它大于 tails 数组所有的值，那么把它添加到 tails 后面，表示最长递增子序列长度加1； 如果 tails[i-1] < x <=
	 * tails[i]，那么更新 tails[i-1] = x。 可以看出 tails 数组保持有序，因此在查找 Si 位于 tails
	 * 数组的位置时就可以使用二分查找。
	 */
	/**
