...
public class RebuildBinaryTree {
	...
	public static TreeNode reConstructBinaryTree(int[] pre, int[] in) {
		...
		TreeNode mm = reConstructBinaryTreeCore(pre, in, 0, pre.length - 1, 0,
				in.length - 1);
		return mm;
	 ...
	 * @return
	 */
	private static TreeNode reConstructBinaryTreeCore(int[] pre, int[] in,
		...
		/* 继续递归构建 */
		if (leifLength > 0) {
			tree.left = reConstructBinaryTreeCore(pre, in, preStart + 1,preStart + leifLength, inStart, r - 1);
		}
		if (rightLength > 0) {
			tree.right = reConstructBinaryTreeCore(pre, in, preStart + 1+ leifLength, preEnd, r + 1, inEnd);
		}
		return tree;
