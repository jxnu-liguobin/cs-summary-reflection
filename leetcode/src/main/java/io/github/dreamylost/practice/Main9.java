 ...
 * 5) 不使用第三方库和java.util.concurrent.BlockingQueue 接口下的实现类
 * @author Mr.Li
 *
 * @param <E>
 */
public class Main9<E> implements BlockingQueue<E> {
	...
	/**
	 * @description 主要得分点如下： 
	 * 1)使用 lock 和 condition 的 signal 机制，或者使用synchronized 和 obj 的 wait/notify 机制的，得 6 分 
	 * 2)队列满和空时，相应方法线程都能正常挂起的，得 6 分，通过 Thread.sleep等其他较为原始的方式实现时，请酌情给分（不高于 2 分）
	 * 3)同步块内队列存储的数据结构读写没有线程安全问题的，得 4分（如 JDK 通过读写分别操作头尾指针，隔离读和写的相互影响，写和读本身各自加锁） 
	 * 4)能对读锁和写锁进行分离提高性能，并且思路正确的，得 2 分 
	 * 5) 使用链表等更适合添加删除操作的数据结构，而非数组，得2 分
	 */
	@Override
